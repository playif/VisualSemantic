...
{
    get maxDepth() {
        return localStorage["max-crawl-depth"] != null ? localStorage["max-crawl-depth"] : 2;
    },
    set maxDepth(val) {
        localStorage['max-crawl-depth'] = val;
    },

    get maxSimultaniousCrawls() {
        return localStorage["max-page-loads"] != null ? localStorage["max-page-loads"] : 10;
    },
    set maxSimultaniousCrawls(val) {
        localStorage['max-page-loads'] = val;
    },

    get pauseOnPopClose() {
        return localStorage["pause-popup-close"] != null ? localStorage["pause-popup-close"] : 1;
    },
    set pauseOnPopClose(val) {
        localStorage['pause-popup-close'] = val;
    },
    ...
    set interestingFileTypes(val) {
        localStorage['interesting-file-types'] = val;
    }
};
...
function onCrawlPageLoaded(page, data) {
    ...
    $(links).each(function () {
        ...
        if (!allPages[absoluteURL]) {
            ...
            if (isInArr(settings.interestingFileTypes, extn)) {
                o.isFile = true;
                counts.interestings++;
            }
...
function getURLsInTab(tab) {
    ...
    for (var ref in allPages) {
        var o = allPages[ref];
        if (tab == "Queued" && o.state == "queued" && !o.isFile) {
            tabUrls.push(o);
        }
        else if (tab == "Crawling" && o.state == "crawling") {
            tabUrls.push(o);
        }
        else if (tab == "Crawled" && o.state == "crawled") {
            tabUrls.push(o);
        }
        else if (tab == "Files" && o.isFile) {
            tabUrls.push(o);
        }
        else if (tab == "Errors" && o.state == "error") {
            tabUrls.push(o);
        }
    }
    return tabUrls;
